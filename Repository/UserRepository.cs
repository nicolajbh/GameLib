          ...
          using (SqlDataReader reader = command.ExecuteReader())
          {
            while (reader.Read())
            {
              ...
              if (!gameMap.TryGetValue(gameId, out Game? game))
              {
                ...
                int developer_id = reader.GetInt32(reader.GetOrdinal("developer_id"));
                string studioName = reader.GetString(reader.GetOrdinal("StudioName"));
                int studioSize = reader.GetInt32(reader.GetOrdinal("studio_size"));
                Developer developer = new Developer(studioName, studioSize, developer_id);
