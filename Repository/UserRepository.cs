            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        const string walletQuery =
                            "INSERT INTO wallet (user_id, balance) VALUES (@USER_ID, @BALANCE)";

                        using (
                            SqlCommand walletCommand = new SqlCommand(
                                walletQuery,
                                connection,
                                transaction
                            )
                        ...
                        {
                            // IMPORTANT: new user ID must be set before reaching this.
                            walletCommand.Parameters.AddWithValue("@USER_ID", newUserId);
                            walletCommand.Parameters.AddWithValue("@BALANCE", 0.00);
    ...
    {
        {
            ...
            {
                Console.WriteLine($"SQL Exception Occurred: {ex.Message}");
                return new Library(user, new List<Game>());
            }
        }
    }
